...
bool thread_wake_tick_cmp(const struct list_elem *a, const struct list_elem *b,
                          void *aux UNUSED) {
    struct thread *t_a = list_entry(a, struct thread, elem);
    struct thread *t_b = list_entry(b, struct thread, elem);
...
// donations 리스트에서 현재 스레드의 wait_on_lock->holder 타고 올라가면서 우선순위
// donate.
void do_donate() {
    int depth;
    struct thread *curr = thread_current();
    ...
    for (depth = 0; depth < 8; depth++) {
        ...
        // *어차피 리스트에 넣을 때 d_elem 우선순위로 내림차순 정렬되어 있으므로 비교할
        // 필요가 없다
...
bool cmp_delem_priority(const struct list_elem *a, const struct list_elem *b,
                        void *aux UNUSED) {
...
void re_dona_priority() {
    ...
    if (!list_empty(&curr->donations)) {
        ...
        struct thread *d_front =
            list_entry(list_front(&curr->donations), struct thread, d_elem);
...
void mlfqs_calc_recent_cpu(struct thread *t) {
    ...
    t->recent_cpu = add_mixed(
        mult_fp(div_fp(mult_mixed(load_avg, 2), add_mixed(mult_mixed(load_avg, 2), 1)),
                t->recent_cpu),
        t->nice);
}
...
void thread_yield(void) {
    ...
    old_level = intr_disable();
    if (curr != idle_thread)
        list_insert_ordered(&ready_list, &curr->elem, cmp_thread_priority,
                            NULL); // 우선순위 기준으로 삽입
