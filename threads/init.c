...
static void run_task(char **argv) { // 작업 실행
    const char *task = argv[1];     // 작업 이름

    printf("Executing '%s':\n", task); // 작업 실행 출력
#ifdef USERPROG
    if (thread_tests) { // 테스트 실행
        run_test(task); // 테스트 실행
    } else {
        process_wait(process_create_initd(task)); // 프로세스 생성 및 대기
    }
#else
    run_test(task); // 테스트 실행
#endif
...
/* ARGV[]에 지정된 모든 액션들을 실행합니다.
   널 포인터 센티널에 도달할 때까지 실행됩니다. */
static void run_actions(char **argv) {
    /* 액션. */
    struct action {
        char *name;                    /* 액션 이름 */
        int argc;                      /* 인자 개수 */
        void (*function)(char **argv); /* 액션 함수 */
    };
